...
std::uint32_t find_graphics_queue(VkPhysicalDevice device) {
    std::uint32_t count = 0;
    ...
    for (std::uint32_t i = 0; i < count; ++i) {
        if (families[i].queueCount && (families[i].queueFlags & VK_QUEUE_GRAPHICS_BIT)) return i;
    }
    return invalid_index;
}

std::uint32_t find_memory_type(VkPhysicalDevice device, std::uint32_t bits, VkMemoryPropertyFlags required) {
...
std::size_t bytes_per_pixel(TextureFormat format) {
    return format == TextureFormat::RGBA16_FLOAT ? 8u : 4u;
}

VkFormat to_vk_vertex_format(VertexFormat format) {
    switch (format) {
...
bool create_surface(VkInstance instance, const NativeWindowHandle& handle, VkSurfaceKHR& surface) {
    switch (handle.type) {
        ...
        case NativeWindowType::UIKit: {
            ...
            auto create_metal = reinterpret_cast<PFN_vkCreateMetalSurfaceEXT>(
                vkGetInstanceProcAddr(instance, "vkCreateMetalSurfaceEXT"));
            return create_metal && create_metal(instance, &info, nullptr, &surface) == VK_SUCCESS;
        }
#endif
        default:
            return false;
...
public:
    VulkanBuffer(VkDevice device, VkPhysicalDevice physical, const BufferDesc& desc)
        ...
        const auto type = find_memory_type(physical, req.memoryTypeBits, flags);
        ...
        if (vkAllocateMemory(device_, &alloc, nullptr, &memory_) != VK_SUCCESS ||
            vkBindBufferMemory(device_, buffer_, memory_, 0) != VK_SUCCESS) destroy();
    }
...
public:
    VulkanTexture(VkDevice device, VkPhysicalDevice physical, VkQueue queue, VkCommandPool pool, const TextureDesc& desc)
        ...
        info.format = to_vk_format(desc_.format);
        ...
        const auto type = find_memory_type(physical_, req.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
        if (type == invalid_index) { destroy(); return; }
        VkMemoryAllocateInfo alloc{VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO};
        alloc.allocationSize = req.size;
        alloc.memoryTypeIndex = type;
        if (vkAllocateMemory(device_, &alloc, nullptr, &memory_) != VK_SUCCESS ||
            vkBindImageMemory(device_, image_, memory_, 0) != VK_SUCCESS) { destroy(); return; }
    ...
    bool upload(const void* data, std::size_t size) override {
        ...
        const auto expected = static_cast<std::size_t>(desc_.width) * desc_.height * bytes_per_pixel(desc_.format);
        ...
        if (vkBeginCommandBuffer(cmd, &begin) == VK_SUCCESS) {
            const auto aspect = desc_.format == TextureFormat::Depth24Stencil8
                ? VK_IMAGE_ASPECT_DEPTH_BIT | VK_IMAGE_ASPECT_STENCIL_BIT : VK_IMAGE_ASPECT_COLOR_BIT;
            ...
            copy.imageExtent = {desc_.width, desc_.height, 1};
            ...
            vkCmdPipelineBarrier(cmd, VK_PIPELINE_STAGE_TRANSFER_BIT, destination_stage,
                                 0, 0, nullptr, 0, nullptr, 1, &barrier);
...
public:
    VulkanSwapchain(VkInstance instance, VkDevice device, VkSurfaceKHR surface, VkSwapchainKHR swapchain,
                    ...
                    VkSemaphore image_available, VkSemaphore render_finished, VkQueue queue)
        : instance_(instance), device_(device), surface_(surface), swapchain_(swapchain), render_pass_(render_pass),
          width_(width), height_(height), images_(std::move(images)), views_(std::move(views)), framebuffers_(std::move(framebuffers)),
          image_available_(image_available), render_finished_(render_finished), queue_(queue) {}
...
public:
    ...
    void end() override {
        if (!recording_ || render_pass_) return;
        executable_ = vkEndCommandBuffer(cmd_) == VK_SUCCESS;
        recording_ = false;
    }
    ...
    void begin_render_pass(const RenderPassDesc& desc) override {
        ...
        if (!swapchain) return;
        render_pass_ = swapchain->begin_render_pass(cmd_, desc.clear_color);
        active_swapchain_ = render_pass_ ? swapchain : nullptr;
    }
    void end_render_pass() override {
        if (!render_pass_ || !active_swapchain_) return;
    ...
    void set_vertex_buffer(Buffer* buffer, std::uint32_t slot) override {
        if (!rendering_ready() || !buffer || slot >= 16) return;
    ...
    void set_index_buffer(Buffer* buffer) override {
        ...
        auto* value = dynamic_cast<VulkanBuffer*>(buffer);
        if (!value) return;
        vkCmdBindIndexBuffer(cmd_, value->handle(), 0, VK_INDEX_TYPE_UINT32);
    ...
    void set_viewport(float x, float y, float width, float height, float min_depth, float max_depth) override {
        if (!rendering_ready()) return;
        VkViewport viewport{x, y, width, height, min_depth, max_depth};
        vkCmdSetViewport(cmd_, 0, 1, &viewport);
    }
    void set_scissor(std::int32_t x, std::int32_t y, std::uint32_t width, std::uint32_t height) override {
        if (!rendering_ready()) return;
    ...
    void draw_indexed(std::uint32_t index_count, std::uint32_t first_index, std::int32_t vertex_offset) override {
        if (!rendering_ready() || !pipeline_bound_ || !index_buffer_bound_ || !index_count) return;
...
public:
    explicit VulkanDevice(const DeviceDesc&) {
        ...
        if (has_instance_extension(VK_KHR_SURFACE_EXTENSION_NAME)) extensions.push_back(VK_KHR_SURFACE_EXTENSION_NAME);
#if defined(VK_USE_PLATFORM_WIN32_KHR)
        if (has_instance_extension(VK_KHR_WIN32_SURFACE_EXTENSION_NAME)) extensions.push_back(VK_KHR_WIN32_SURFACE_EXTENSION_NAME);
#endif
#if defined(VK_USE_PLATFORM_XLIB_KHR)
        if (has_instance_extension(VK_KHR_XLIB_SURFACE_EXTENSION_NAME)) extensions.push_back(VK_KHR_XLIB_SURFACE_EXTENSION_NAME);
#endif
#if defined(VK_USE_PLATFORM_WAYLAND_KHR)
        if (has_instance_extension(VK_KHR_WAYLAND_SURFACE_EXTENSION_NAME)) extensions.push_back(VK_KHR_WAYLAND_SURFACE_EXTENSION_NAME);
#endif
#if defined(VK_USE_PLATFORM_ANDROID_KHR)
        if (has_instance_extension(VK_KHR_ANDROID_SURFACE_EXTENSION_NAME)) extensions.push_back(VK_KHR_ANDROID_SURFACE_EXTENSION_NAME);
#endif
#if defined(VK_USE_PLATFORM_METAL_EXT)
        if (has_instance_extension(VK_EXT_METAL_SURFACE_EXTENSION_NAME)) extensions.push_back(VK_EXT_METAL_SURFACE_EXTENSION_NAME);
#endif
        ...
        for (auto candidate : devices) {
            const auto family = find_graphics_queue(candidate);
            if (family != invalid_index) {
                physical_ = candidate;
                graphics_family_ = family;
                swapchain_supported_ = has_device_extension(candidate, VK_KHR_SWAPCHAIN_EXTENSION_NAME);
                break;
            }
        }
        if (!physical_) return;
    ...
    std::unique_ptr<Pipeline> create_pipeline() override { return nullptr; }
    ...
    std::unique_ptr<Pipeline> create_pipeline(const GraphicsPipelineDesc& desc) override {
        ...
        for (const auto& attribute : desc.vertex_attributes) {
            attributes.push_back({attribute.location, attribute.binding, to_vk_vertex_format(attribute.format), attribute.offset});
        }
        ...
        stages[0] = {VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO, nullptr, 0, VK_SHADER_STAGE_VERTEX_BIT, vertex_module, "main", nullptr};
        stages[1] = {VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO, nullptr, 0, VK_SHADER_STAGE_FRAGMENT_BIT, fragment_module, "main", nullptr};
        ...
        const auto result = vkCreateGraphicsPipelines(device_, VK_NULL_HANDLE, 1, &pipeline_info, nullptr, &pipeline);
        vkDestroyShaderModule(device_, fragment_module, nullptr);
        vkDestroyShaderModule(device_, vertex_module, nullptr);
    ...
    std::unique_ptr<Swapchain> create_swapchain(const SwapchainDesc& desc) override {
        ...
        if (vkGetPhysicalDeviceSurfaceSupportKHR(physical_, graphics_family_, surface, &supported) != VK_SUCCESS || !supported) {
            vkDestroySurfaceKHR(instance_, surface, nullptr);
            return nullptr;
        }

        std::uint32_t format_count = 0, mode_count = 0;
        ...
        const auto requested = to_vk_format(desc.format);
        for (const auto& candidate : formats) {
            if (candidate.format == requested && candidate.colorSpace == VK_COLOR_SPACE_SRGB_NONLINEAR_KHR) { format = candidate; break; }
        }
        VkPresentModeKHR present_mode = VK_PRESENT_MODE_FIFO_KHR;
        for (auto candidate : modes) if (candidate == VK_PRESENT_MODE_MAILBOX_KHR) { present_mode = candidate; break; }
        VkExtent2D extent{};
        ...
        for (auto image : images) {
            ...
            if (vkCreateImageView(device_, &view, nullptr, &handle) != VK_SUCCESS) {
                for (auto value : views) vkDestroyImageView(device_, value, nullptr);
                vkDestroySwapchainKHR(device_, swapchain, nullptr);
                vkDestroySurfaceKHR(instance_, surface, nullptr);
                return nullptr;
            }
        ...
        if (vkCreateRenderPass(device_, &render_info, nullptr, &render_pass) != VK_SUCCESS) {
            for (auto value : views) vkDestroyImageView(device_, value, nullptr);
            vkDestroySwapchainKHR(device_, swapchain, nullptr);
            vkDestroySurfaceKHR(instance_, surface, nullptr);
            return nullptr;
        ...
        for (auto view : views) {
            ...
            if (vkCreateFramebuffer(device_, &fb, nullptr, &handle) != VK_SUCCESS) {
    ...
    bool submit(CommandBuffer& commands, Swapchain* swapchain) override {
        ...
        if (!value || !value->executable()) return false;
        auto* target = dynamic_cast<VulkanSwapchain*>(swapchain);
        VkSemaphore wait = target ? target->image_available() : VK_NULL_HANDLE;
        VkSemaphore signal = target ? target->render_finished() : VK_NULL_HANDLE;
        VkPipelineStageFlags wait_stage = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT;
        VkSubmitInfo submit_info{VK_STRUCTURE_TYPE_SUBMIT_INFO};
        submit_info.waitSemaphoreCount = wait ? 1u : 0u;
        submit_info.pWaitSemaphores = wait ? &wait : nullptr;
        submit_info.pWaitDstStageMask = wait ? &wait_stage : nullptr;
        submit_info.commandBufferCount = 1;
        const auto cmd = value->handle();
        submit_info.pCommandBuffers = &cmd;
        submit_info.signalSemaphoreCount = signal ? 1u : 0u;
        submit_info.pSignalSemaphores = signal ? &signal : nullptr;
        if (vkQueueSubmit(queue_, 1, &submit_info, VK_NULL_HANDLE) != VK_SUCCESS) return false;
